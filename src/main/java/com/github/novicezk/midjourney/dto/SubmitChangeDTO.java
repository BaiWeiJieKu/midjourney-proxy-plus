...
public class SubmitChangeDTO extends BaseSubmitDTO {
	...
	@ApiModelProperty(value = "UPSCALE(放大); VARIATION(变换); REROLL(重新生成); UPSAMPLE2X(像素放大2倍); UPSAMPLE4X(像素放大4倍)", required = true,
			allowableValues = "UPSCALE, VARIATION, REROLL, UPSAMPLE2X, UPSAMPLE4X", example = "UPSCALE")
