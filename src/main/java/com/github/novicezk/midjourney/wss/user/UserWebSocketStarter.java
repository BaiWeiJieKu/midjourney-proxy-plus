...
public class UserWebSocketStarter extends WebSocketAdapter implements WebSocketStarter {
	...
	public synchronized void start() throws Exception {
		log.info("WebSocket start==============start===============");
		...
		this.socket.connect();
		log.info("WebSocket start url:{}",gatewayUrl + "/?encoding=json&v=9&compress=zlib-stream");
		log.info("WebSocket start==============end===============");
	}

	@Override
	public void onConnected(WebSocket websocket, Map<String, List<String>> headers) {
		log.info("WebSocket连接[wss-{}] Connected to websocket.", this.account.getDisplay());
	}

	@Override
	public void handleCallbackError(WebSocket websocket, Throwable cause) throws Exception {
		log.error("WebSocket连接[wss-{}] There was some websocket error.", this.account.getDisplay(), cause);
	}
	...
	public void onBinaryMessage(WebSocket websocket, byte[] binary) throws Exception {
		...
		//log.info("WebSocket onBinaryMessage messageJson:{}",json);
	...
	private void handleDispatch(DataObject raw) {
		...
		if ("READY".equals(t)) {
			...
			log.info("WebSocket连接 [wss-{}] Dispatch ready: identify.sessionId：{},resumeGatewayUrl：{}", this.account.getDisplay(),this.sessionId,this.resumeGatewayUrl);
			connectSuccess();
			return;
		} else if ("RESUMED".equals(t)) {
			log.info("WebSocket连接 [wss-{}] Dispatch read: resumed.", this.account.getDisplay());
			connectSuccess();
			return;
		}
		try {
			log.info("WebSocket handleDispatch--->userMessageListener.onMessage");
